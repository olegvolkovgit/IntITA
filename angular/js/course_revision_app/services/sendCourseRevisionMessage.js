 ...
 */
angular
    .module('courseRevisionSendMessage',[])
    .service('courseRevisionMessage', [
        '$http',
        function($http) {
            var self = this;
            self.sendMessage = function(idRevision) {
                $http({
                    url: basePath+'/courseRevision/getDataForCourseRevisionMail',
                    method: "POST",
                    ...
                    bootbox.dialog({
                            ...
                            '<div class="form-group col-md-12">'+
                            '<label>Лист</label>'+
                            '<textarea class="form-control" style="resize: none" rows="6" id="messageText" required>Посилання на ревізію курса: '+basePath+response.data.link+'</textarea>'+
                            '</div>'+
                            '</form>'+
                            '</div>'+
                            '</div>',
