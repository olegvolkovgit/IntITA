...
function taskCtrl($rootScope,$http, $scope, accessLectureService,openDialogsService,startJson,ipCookie) {
    $scope.sendTaskAnswer=function(jobid, idTask, taskLang, url,e){
        ...
        if($.trim(answer) == '')
        {
            ...
            startJson.startJson(url, taskLang, idTask, $scope.userCode, ipCookie("PHPSESSID"), jobid).then(function (response) {
                $('#ajaxLoad').hide();
                alert(response);
                button.removeAttr('disabled');
            });
