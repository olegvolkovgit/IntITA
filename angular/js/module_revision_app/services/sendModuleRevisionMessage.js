 ...
 */
angular
    .module('moduleRevisionSendMessage',[])
    .service('moduleRevisionMessage', [
        '$http',
        function($http) {
            var self = this;
            self.sendMessage = function(idRevision) {
                $http({
                    url: basePath+'/moduleRevision/getDataForModuleRevisionMail',
                    method: "POST",
                    data: $.param({idRevision: idRevision}),
                    headers: {'Content-Type': 'application/x-www-form-urlencoded;charset=utf-8;'}
                }).then(function successCallback(response) {
                    bootbox.dialog({
                            title: "Надіслати листа автору ревізії модуля: "+response.data.authorName,
                            message:
                            ...
                            '<div class="form-group col-md-12">'+
                            '<label>Лист</label>'+
                            '<textarea class="form-control" style="resize: none" rows="6" id="messageText" required>Посилання на ревізію модуля: '+basePath+response.data.link+'</textarea>'+
                            '</div>'+
                            '</form>'+
                            '</div>'+
                            '</div>',
