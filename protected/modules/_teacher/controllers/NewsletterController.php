...
class NewsletterController extends TeacherCabinetController
{
    ...
    public function actionSendLetter()
    {
        ...
        if ($task->repeat_type == SchedulerTasks::WEEKDAYS){
            $task->setScenario('typeWeekdays');
        }
        if ($newsLetter->validate() && $task->validate()) {
            $newsLetter->save();
        ...
        echo json_encode(array('success'=>0, 'errors'=>array_merge($task->getErrors(), $newsLetter->getErrors())));
        Yii::app()->end();
