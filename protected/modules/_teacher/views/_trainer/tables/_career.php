<div  ng-controller="careerStudentsCtrl">
    <div >
        <div class="dataTable_wrapper">
            <table ng-table="studentCareerInfoTableParams" class="table table-bordered table-striped table-condensed" style="margin-right: 20px;">
                <colgroup>
                    <col width="60px"/>
                    <col width="70px"/>
                    <col width="80px"/>
                ...
                <tr ng-repeat="row in $data track by $index">
                    <td data-title="'Примітки'" filter="{'notes': 'text'}" sortable="'notes'">
                        <a href="" ng-click="updateCareerInfo(row.id_student, 'notes')">{{ row.notes ? row.notes : "Редагувати" }}</a>
                    </td>
                    <td style="word-wrap:break-word" data-title="'Імя'" filter="{'first_name': 'text'}" sortable="'first_name'">
                        <a ng-href="" ng-click="updateCareerInfo(row.id_student ,'first_name')">{{ row.first_name ? row.first_name : 'Редагувати' }}</a>
                    </td>
                    <td data-title="'По-батькові'" filter="{'middle_name': 'text'}" sortable="'middle_name'">
                        <a ng-href="" ng-click="updateCareerInfo(row.id_student ,'middle_name')">{{ row.middle_name ? row.middle_name : 'Редагувати' }}</a>
                    </td>
                    <td data-title="'Прізвище'" filter="{'second_name': 'text'}" sortable="'second_name'">
                        <a ng-href="" ng-click="updateCareerInfo(row.id_student ,'second_name')">{{ row.second_name ? row.second_name : 'Редагувати' }}</a>
                    </td>
                    <td data-title="'Місце роботи'" filter="{'current_job': 'text'}" sortable="'current_job'">
                        <a ng-href="" ng-click="updateCareerInfo(row.id_student ,'current_job')">{{ row.current_job ? row.current_job : 'Редагувати' }}</a>
                    </td>
                    <td data-title="'Попереднє місце роботи'" filter="{'prev_job': 'text'}" sortable="'prev_job'">
                        <a ng-href="" ng-click="updateCareerInfo(row.id_student ,'prev_job')">{{ row.prev_job ? row.prev_job : 'Редагувати' }}</a>
                    </td>
                    <td data-title="'Освіта'" filter="{'education': 'text'}" sortable="'education'">
                        <a ng-href="" ng-click="updateCareerInfo(row.id_student ,'education')">{{row.education ? row.education : 'Редагувати' }}</a>
                    </td>
                    <td data-title="'Рівень англійської мови'" filter="{'english_level': 'text'}" sortable="'english_level'">
                        <a ng-href="" ng-click="updateCareerInfo(row.id_student ,'english_level')">
