<ul style="list-style: none;padding:5px;">
    <li ng-repeat="task in tasks track by $index">
        ...
        <div ng-if="task.event=='task'">
            <a href="#/users/profile/{{task.idTask.created_by}}" target="_blank">{{task.idTask.createdBy.fullName}}</a>{{!task.idTask.change_date?' створив завдання: ':' відредагував завдання: '}}
        ...
        <div ng-if="task.event=='comment'">
            <a href="#/users/profile/{{task.id_user}}" target="_blank">{{task.idUser.fullName}}</a> додав коментар  "<b><em ng-bind-html="task.message | htmlToPlaintext | limitTo:50"></em></b>" до завдання
            <a href="" ng-click="getTask(task.idTask.id)">{{task.idTask.name}}</a>, {{task.create_date}}
            <hr>
        </div>
        <div ng-if="task.event=='role'">
            <a href="#/users/profile/{{task.assigned_by}}" target="_blank">{{task.assignedBy.fullName}}</a> {{!task.cancelled_date?' призначив вам роль ':' скасував у вас роль '}}
        ...
        <div ng-if="task.event=='state'">
            <a ng-href="#/users/profile/{{task.id_user}}" target="_blank">{{task.idUser.fullName}}</a> перевів завдання
