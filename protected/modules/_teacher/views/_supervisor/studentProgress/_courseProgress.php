    ...
    <div class="panel panel-default">
        <div class="panel-body" class="ng-cloak">
            <div class="row" ng-repeat="row in data">
                <label class="progress-labe col-sm-4"style="float: left;"><a ng-attr-class="{{(row.progress.passedLectures == 0 && !row.progress.isDone ) && 'disabled'}}" ui-sref="students/moduleProgress/:studentId/:module({studentId:student.id,module:row.idModule})">Модуль: {{row.module}} </a> </label>
                <div class="col-sm-6"><uib-progressbar  max="((row.progress.lectures > 0)) && row.progress.lectures || 99999 "
                    ...
                    >{{(row.progress.isDone) && 'Завершено' || 'Пройдено лекцій ' + row.progress.passedLectures + ' з ' + row.progress.lectures }}
                        </uib-progressbar></div>
            </div>
        </div>
    </div>
