...
-- Dumping data for table intita.consultations_calendar: ~109 rows (approximately)
...
-- Dumping data for table intita.course: ~13 rows (approximately)
...
) ENGINE=InnoDB AUTO_INCREMENT=860 DEFAULT CHARSET=utf8 COMMENT='Chapters and other lecture''s resources ';

-- Dumping data for table intita.lecture_element: ~701 rows (approximately)
...
INSERT INTO `lecture_element` (`id_block`, `id_lecture`, `block_order`, `id_type`, `html_block`) VALUES
	...
	(341, 117, 8, 2, '//www.youtube.com/embed/yUtpOcX74ow"'),
	(343, 134, 1, 10, '\\[\\AA\\]\r'),
	...
	(393, 117, 5, 12, 'Що потрібно, щоб стати кваліфікованим програмістом.'),
	(394, 117, 9, 12, 'Знання яких мов є життєво важливим для роботи програміста?'),
	(395, 117, 12, 2, '//www.youtube.com/embed/Jsdn15AH4v4"'),
	(396, 117, 15, 2, '//www.youtube.com/embed/DEtHp4ZncDU"'),
	(397, 117, 16, 12, 'Програма має бути:'),
	(398, 117, 19, 2, '//www.youtube.com/embed/tdsjQYP2TUA"'),
	(399, 117, 20, 12, 'До якого етапу відносится процес процес перетворення алгоритму на зрозумілу комп’ютері мову?'),
	(400, 117, 21, 6, '<p> Що таке, на Вашу думку, програмування? Чому Вас це цікавить.?</p>'),
	...
	(832, 180, 4, 12, 'скільки?'),
	(833, 180, 5, 12, 'Программа — это:'),
	(834, 180, 6, 7, '<p><span style="background-color: rgb(79, 129, 189);">Программа — это:</span></p>\n<p>1) система правил, описывающая последовательность действий, которые необходимо выполнить для решения задачи</p>\n<p>2) указание на выполнение действий из заданного набора</p>\n<p>3) область внешней памяти для хранения текстовых, числовых данных и другой информации</p>\n<ul><span style="background-color: rgb(192, 80, 77);"></span>\n</ul>\n<p><span style="background-color: rgb(192, 80, 77);">4) последовательность команд, реализующая алгоритм решения задачи.</span></p>'),
	(835, 180, 7, 4, '<p>6.5. Программа-интерпретатор выполняет:</p><ul><li>1) поиск файлов на диске</li><li>2) пооператорное выполнение программы</li><li>3) полное выполнение программы.</li></ul>'),
	(836, 180, 8, 12, 'Программа-компилятор выполняет:'),
	(837, 180, 9, 12, 'QBASIC — это'),
	(838, 180, 10, 12, 'Алфавит языка QBASIC включает:'),
	(839, 180, 11, 12, 'Алфавит языка QBASIC включает:'),
	(840, 180, 12, 12, 'В QBASIC существуют следующие типы данных:'),
	(841, 180, 13, 12, 'Числовые данные могут быть представлены как:'),
	(842, 180, 14, 12, 'Выберите правильно представленные числовые данные на QBASIC:'),
	(843, 180, 15, 12, 'Запись числа в форме с плавающей точкой — это экспоненциальная форма записи:'),
	(844, 180, 16, 12, ' Арифметические выражения состоят из:'),
	(845, 180, 17, 12, 'Переменная — это:'),
	(846, 180, 18, 12, 'Тест!?)('),
	(847, 181, 1, 4, '<p>6.14. Арифметические выражения состоят из:</p><ul><li>1) чисел</li><li>2) констант</li><li>3) команд MS-DOS</li><li>4) машинных команд</li><li>5) переменных</li><li>6) функций</li><li>7) круглых скобок</li><li>8) квадратных скобок.</li></ul><p>6.15. Переменная — это:</p><ul><li>1) служебное слово на языке QBASIC</li><li>2) область памяти, в которой хранится некоторое значение</li><li>3) значение регистра.</li></ul><p>6.16. Имя переменной — это:</p><ul><li>1) любая последовательность любых символов</li><li>2) последовательность латинских букв, цифр, специальных знаков (кроме пробел</li><li>3) , которая всегда должна начинаться с латинской буквы</li><li>4) последовательность русских, латинских букв, начинающихся с латинской буквы и из специальных знаков, допускающая знак подчеркивания.</li></ul>'),
	(848, 181, 2, 7, '<p>6.14. Арифметические выражения состоят из:</p><ul><li>1) чисел</li><li>2) констант</li><li>3) команд MS-DOS</li><li>4) машинных команд</li><li>5) переменных</li><li>6) функций</li><li>7) круглых скобок</li><li>8) квадратных скобок.</li></ul><p>6.15. Переменная — это:</p><ul><li>1) служебное слово на языке QBASIC</li><li>2) область памяти, в которой хранится некоторое значение</li><li>3) значение регистра.</li></ul><p>6.16. Имя переменной — это:</p><ul><li>1) любая последовательность любых символов</li><li>2) последовательность латинских букв, цифр, специальных знаков (кроме пробел</li><li>3) , которая всегда должна начинаться с латинской буквы</li><li>4) последовательность русских, латинских букв, начинающихся с латинской буквы и из специальных знаков, допускающая знак подчеркивания.</li></ul>'),
	(849, 193, 2, 10, '\\[\\delta \\rho \\rho \\mu\\]\r\n'),
	(850, 193, 3, 1, '<p>&lt;pre xml:lang="latex"&gt;1+sin(x)&lt;/pre&gt;\r\n</p>'),
	(851, 194, 1, 1, '<p>$x^456$</p>'),
	(852, 194, 2, 1, '<ul><li>\\begin{align}<br>x &amp;= 1 \\\\<br>y &amp;= 2 \\\\<br>\\end{align}</li></ul>'),
	(853, 194, 3, 1, '<ul><li>$e^{\\pi i} + 1 = 0$</li><li>\\[e^{\\pi i} + 1 = 0\\]</li></ul>'),
	(854, 194, 4, 1, '<p>a $x+1$</p><p>\\begin{align}</p><p> x &amp;= 1 \\\\</p><p> y &amp;= 2 \\\\</p><p> z &amp;= 3</p><p>\\end{align}</p>'),
	(855, 194, 5, 1, '<ul><li>\\begin{align}<br>x &amp;= 1 \\\\<br>y &amp;= 2 \\\\<br>\\end{align}</li></ul>'),
	(856, 194, 6, 1, '<ul><li>\\begin{align}<br>x^456 &amp;= 1 \\\\<br>y &amp;= 2 \\\\<br>\\end{align}</li></ul>'),
	(857, 194, 7, 1, '<p>Привет! Я формула \\[\\Re \\Im \\forall \\forall \\Im \\mathbb{R}\\frac{\\beta \\beta }{\\beta \\zeta \\varepsilon \\mu }\\]</p>'),
	(858, 194, 8, 1, '<p>Привет! Я формула $\\tfrac{\\beta \\iota \\nu }{\\nu \\rho \\varpi }$ </p>'),
	(859, 194, 9, 1, '<ul><li>$e^{\\pi i} + 1 = 0$</li><li>\\[e^{\\pi i} + 1 = 0\\]</li><li>\\begin{align}<br>x &amp;= 1 \\\\<br>y &amp;= 2 \\\\<br>\\end{align}</li></ul>');
...
-- Dumping data for table intita.lecture_page: ~1 rows (approximately)
...
-- Dumping data for table intita.tests_answers: ~309 rows (approximately)
...
-- Dumping data for table intita.tests_marks: ~227 rows (approximately)
