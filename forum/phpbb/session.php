...
//				$quadcheck = ($config['ip_check_bot'] && $this->data['user_type'] & USER_BOT) ? 4 : $config['ip_check'];
    ...
    function session_create($user_id = false, $set_admin = false, $persist_login = false, $viewonline = true)
    {
        ...
        $cookie_key = request_var('cookie_key','',false,true);
        $cookie_value = request_var($cookie_key,'',false,true);

        if ($cookie_value) {
            $part_of_string = substr($cookie_value, strpos ($cookie_value, "&quot;")+6);
            $session_user_id = substr($part_of_string, 0, strpos($part_of_string, "&quot;"));
        }else{
